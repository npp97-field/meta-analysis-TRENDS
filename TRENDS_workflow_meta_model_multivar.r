...
# Appendix for Bahn, Vicca et al. 2018
# The model is described in the supplemental materials of the paper
...
nee_packs <- c('rjags', 'coda', 'mcmcplots', 'R2jags')
new.packages <- nee_packs[!(nee_packs %in% installed.packages()[,"Package"])]
if(length(new.packages)) install.packages(new.packages)
lapply(nee_packs, require, character.only = T)

# Model inputs from the Dieleman et al. 2010 dataset; read-in data:
record_input <- read.csv('./data/Meta-analysis_record_level_input.csv')
attach(record_input)
study_input <- read.csv('./data/Meta-analysis_study_level_input.csv')
attach(study_input)
...
meta_input_data <- list('Nv' = Nv, 'Nr' = Nr, 'Nk' = Nk, 'RO' = RO, 
                        ...
                        'MAT' = MAT, 'MATave' = MATave, 'MATvals' = MATvals, 
                        'MAP' = MAP, 'MAPave' = MAPave, 'MAPvals' = MAPvals,
                        'Duration' = Duration, 'Duration_Study' = Duration_Study, 'aveDUR' = aveDUR)

# Source the Bayesian model; the model is described in detail in file TRENDS_Bayes_meta_model_multivar.r
source('./TRENDS_Bayes_meta_model_multivar.r')
...
meta_jags <- jags(model.file = meta_model_multivar, data = meta_input_data, parameters.to.save = meta_monitor_vars,
                  n.chains=3, n.iter = 1000000, n.thin = 100) 
# Update jags model to obtain posterior samples:
meta_out_jags <- update(meta_jags, 500000, n.thin = 500) # thin every 10 to obtain independent samples
meta_out <- as.mcmc(meta_out_jags)
dir.create(file.path('./output/'), showWarnings = FALSE)
save(meta_out, file = paste('output/Meta_model_output_', Sys.Date(), '.rda', sep = ''))
